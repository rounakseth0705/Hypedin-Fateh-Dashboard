...
export default function AmbassadorSidebar({ isOpen, onClose }) {
  const navigate = useNavigate();
  const location = useLocation();

  const menuItems = [
    { name: "Dashboard", path: "/ambassador", icon: LayoutDashboard },
    { name: "Tasks", path: "/ambassador/tasks", icon: CheckSquare },
    { name: "Inbox", path: "/ambassador/inbox", icon: Inbox },
    { name: "Profile", path: "/ambassador/profile", icon: User }
  ];
    ...
    <>
      ...
      `}>
        ...
        <div className="p-6 border-b border-[#dadce0] space-y-2">
          ...
          <h2 className="text-sm font-bold text-[#202124]">Ambassador Portal</h2>
        </div>
